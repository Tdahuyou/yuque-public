...
n. 压缩，浓缩；压榨，压迫
`concentration` `pressure` `hitting`

# 同根

- adj. `compressed` 被压缩的；扁平的
- adj. `compressible` 可压缩的；可压榨的
- n. `compressor` 压缩机；压缩物；收缩肌；[医] 压迫器
- n. `compressibility` 压缩性；压缩系数；压缩率
- vi. `compress` 受压缩小
- vt. `compress` 压缩，压紧；精简
...
Custom programming of every compression is not always desirable or possible.
但是，每次压缩定制编程并非总是值得或可能的。
