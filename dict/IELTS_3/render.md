...
- n. `rendering` 翻译；表现；表演；描写；打底；（建筑物等）透视图
- vi. `rendering` 给予补偿（rendr的ing形式）
- vt. `rendering` 致使；表演；打底（render的ing形式）
